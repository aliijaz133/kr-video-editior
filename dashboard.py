...
class Dashboard:
    def __init__(self, master):
        ...
        self.video_format_dropdown = tk.OptionMenu(
            self.master, self.video_format_var, *self.video_format_options
        )
        self.video_format_dropdown.grid(row=1, column=1, padx=10, pady=5, sticky=tk.W)

        self.create_project_button = tk.Button(
            self.master, text="Create Project", command=self.create_project
        )
    ...
    def create_project(self):
        ...
        if project_name:
            ...
            subprocess.run(
                ["python", "video-editor.py", project_name, video_format]
            )  # Pass project name as command-line argument
            self.master.destroy()
        else:
